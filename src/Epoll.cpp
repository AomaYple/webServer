...
Epoll::Epoll(const source_location &sourceLocation) : self(epoll_create1(0)), epollEvents(1024) {
    if (this->self == -1)
...
auto Epoll::poll(bool block, const source_location &sourceLocation) -> pair<const vector<epoll_event> &, unsigned short> {
...
auto Epoll::add(int fileDescriptor, uint32_t event, const source_location &sourceLocation) const -> void {
