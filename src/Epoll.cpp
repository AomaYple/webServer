...
Epoll::Epoll(const source_location &sourceLocation) : fileDescriptor{epoll_create1(0)}, epollEvents{1024} {
    if (this->fileDescriptor == -1)
        Log::add(sourceLocation, Level::ERROR, "Epoll create error: " + string(strerror(errno)));
}

Epoll::Epoll(Epoll &&epoll) noexcept : fileDescriptor{epoll.fileDescriptor}, epollEvents{std::move(epoll.epollEvents)} {
...
auto Epoll::poll(bool block, const source_location &sourceLocation)
    ...
    int eventNumber{epoll_wait(this->fileDescriptor, this->epollEvents.data(),
                               static_cast<int>(this->epollEvents.size()), block ? -1 : 0)};
...
auto Epoll::add(int newFileDescriptor, uint32_t event, const source_location &sourceLocation) const -> void {
    epoll_event epollEvent{event};

    epollEvent.data.fd = newFileDescriptor;

    if (epoll_ctl(this->fileDescriptor, EPOLL_CTL_ADD, newFileDescriptor, &epollEvent) == -1)
