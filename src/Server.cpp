...
using std::string, std::to_string, std::vector, std::shared_ptr,
    std::make_shared, std::source_location;
...
Server::Server(unsigned short port, const source_location& sourceLocation)
  ...
  if (this->socket == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Server create error: " + string{strerror(errno)});

  int option{1};
  if (setsockopt(this->socket, SOL_SOCKET, SO_REUSEADDR, &option,
                 sizeof(option)) == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Server reuseAddress error: " + string{strerror(errno)});

  if (setsockopt(this->socket, SOL_SOCKET, SO_REUSEPORT, &option,
                 sizeof(option)) == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Server reusePort error: " + string{strerror(errno)});
  ...
  if (inet_pton(AF_INET, "127.0.0.1", &address.sin_addr) != 1)
    Log::add(sourceLocation, Level::ERROR,
             "Server translate ipAddress error: " + string{strerror(errno)});

  if (::bind(this->socket, reinterpret_cast<sockaddr*>(&address),
             addressLength) == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Server bind error: " + string{strerror(errno)});

  if (::listen(this->socket, SOMAXCONN) == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Server listen error: " + string{strerror(errno)});
}

Server::Server(Server&& server) noexcept
    : socket{server.socket}, idleFileDescriptor{server.idleFileDescriptor} {
  server.socket = -1;
...
auto Server::accept(source_location sourceLocation)
    -> vector<shared_ptr<Client>> {
  ...
  while (true) {
    ...
    int fileDescriptor{accept4(this->socket,
    ...
    if (fileDescriptor != -1) {
      ...
      if (inet_ntop(AF_INET, &address.sin_addr, information.data(),
                    information.size()) == nullptr)
        Log::add(
            sourceLocation, Level::ERROR,
            "Client translate ipAddress error: " + string{strerror(errno)});
      ...
      if (errno == EMFILE) {
        ...
        Log::add(sourceLocation, Level::ERROR,
                 "Server accept error: " + string{strerror(errno)});
...
Server::~Server() {
  if (this->idleFileDescriptor != -1 && close(this->idleFileDescriptor) == -1)
    Log::add(
        source_location::current(), Level::ERROR,
  ...
  if (this->socket != -1 && close(this->socket) == -1)
    Log::add(source_location::current(), Level::ERROR,
             "Server close error: " + string{strerror(errno)});
}
