...
using std::string_view, std::pair, std::vector;

Buffer::Buffer() : self{vector<char>(1024, 0)}, start{0}, end{0} {}

Buffer::Buffer(Buffer&& buffer) noexcept
    : self{std::move(buffer.self)}, start{buffer.start}, end{buffer.end} {
...
auto Buffer::write(const string_view& data) -> void {
  if (data.size() > this->self.size() - this->end && this->start > 0) {
    std::copy(this->self.begin() + this->start, this->self.begin() + this->end,
              this->self.begin());
