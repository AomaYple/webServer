...
Timer::Timer(const source_location& sourceLocation)
  ...
  if (this->fileDescriptor == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Timer create error: " + string{strerror(errno)});

  itimerspec time{{1, 0}, {1, 0}};

  if (timerfd_settime(this->fileDescriptor, 0, &time, nullptr) == -1)
    Log::add(sourceLocation, Level::ERROR,
             "Timer setTime error: " + string{strerror(errno)});
}
...
auto Timer::add(const shared_ptr<Client>& client) -> void {
  unsigned short clientLocation{
      static_cast<unsigned short>(this->now + client->getTimeout())};
...
auto Timer::handleReadableEvent(const source_location& sourceLocation) -> void {
  ...
  if (read(this->fileDescriptor, &expireNumber, sizeof(expireNumber)) ==
      sizeof(expireNumber)) {
    ...
    Log::add(sourceLocation, Level::ERROR,
             "Timer read error: " + string{strerror(errno)});
}
...
Timer::~Timer() {
  ...
  if (this->fileDescriptor != -1 && close(this->fileDescriptor) == -1)
    Log::add(source_location::current(), Level::ERROR,
             "Timer close error: " + string{strerror(errno)});
}
