...
using std::cout, std::string, std::array, std::ostringstream, std::put_time, std::jthread, std::this_thread::get_id,
    std::atomic_ref, std::memory_order_relaxed, std::memory_order_release, std::chrono::system_clock, std::source_location,
    std::format;
...
Log::Log() : head {new Node {}}, stop {false}, work {[this] {
    while (!this->stop) {
        ...
        while (this->head != this->tail.load(memory_order_relaxed)) {
            ...
            cout << format("{}:{}:{}:{} {} {}\n", data.sourceLocation.file_name(), data.sourceLocation.line(),
                           data.sourceLocation.column(), data.sourceLocation.function_name(), levels[static_cast<int>(data.level)],
                           data.information);
...
Log::Node::Message::Message() : time {system_clock::now()}, threadId {get_id()}, sourceLocation {source_location::current()},
    level {Level::INFO} {}

Log::Node::Message::Message(system_clock::time_point time, jthread::id threadId, std::source_location sourceLocation,
                            Level level, string &&information) : time {time}, threadId {threadId}, sourceLocation {sourceLocation},
                            level {level}, information {std::move(information)} {}

Log::Node::Message::Message(Message &&message) noexcept : time {message.time}, threadId {message.threadId}, sourceLocation {message.sourceLocation},
    level {message.level}, information {std::move(message.information)} {}
