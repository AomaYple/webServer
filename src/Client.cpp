...
Client::Client(int fileDescriptor, string information, unsigned short timeout,
  ...
  if (setsockopt(this->socket, SOL_SOCKET, SO_LINGER, &linger,
                 sizeof(linger)) == -1)
    Log::add(sourceLocation, Level::ERROR,
             this->information + " location linger error: " + strerror(errno));
}
  ...
  while (true) {
    ...
    if (receivedBytes > 0) {
      ...
      if (errno != EAGAIN && errno != EWOULDBLOCK) {
        ...
        Log::add(sourceLocation, Level::ERROR,
                 this->information + " receive error: " + strerror(errno));
      }
...
auto Client::send(const source_location& sourceLocation) -> void {
  ...
  while (allSentBytes < data.size()) {
    ssize_t sentBytes{::write(this->socket, data.data() + allSentBytes,
                              data.size() - allSentBytes)};
    ...
    if (sentBytes > 0)
      ...
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        ...
        Log::add(sourceLocation, Level::INFO,
                 " too much data can not be sent to " + this->information);
      } else {
        this->event = 0;

        Log::add(sourceLocation, Level::ERROR,
                 this->information + " send error: " + strerror(errno));
      }
...
auto Client::getEvent() const -> unsigned int {
  return this->event;
}
  ...
  if (this->socket != -1) {
    if (close(this->socket) == -1)
      Log::add(source_location::current(), Level::ERROR,
               this->information + " close error: " + strerror(errno));
    else
      Log::add(source_location::current(), Level::INFO,
               this->information + " is closed");
  }
}
