...
EventLoop::EventLoop(unsigned short port, bool startThread) : server{port} {
  auto function{[this] {
    ...
    while (true) {
      ...
      for (auto event{events.first.begin()};
           event != events.first.begin() + events.second; ++event) {
...
auto EventLoop::handleClientEvent(int fileDescriptor, uint32_t event,
  ...
  if (client != nullptr) {
    if (event & (EPOLLERR | EPOLLHUP | EPOLLRDHUP))
      ...
      Log::add(sourceLocation, Level::ERROR,
               string{client->getInformation()} + " unknown event");
...
auto EventLoop::handleClientReceivableEvent(shared_ptr<Client>& client)
    -> void {
  client->receive();
