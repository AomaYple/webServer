...
Log::Log(Log::Level level, std::string &&text, std::source_location sourceLocation,
         std::chrono::system_clock::time_point timestamp, std::jthread::id threadId) noexcept
    : level{level}, timestamp{timestamp}, threadId{threadId}, sourceLocation{sourceLocation}, text{std::move(text)} {}
