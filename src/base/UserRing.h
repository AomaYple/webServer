...
public:
    ...
    auto operator=(UserRing &&)

            noexcept -> UserRing &;

    [[nodiscard]] auto getSelfFileDescriptor() const
    ...
    [[nodiscard]] auto forEachCompletion(const

                                         std::function<auto(io_uring_cqe *cqe)->void> &task) -> unsigned int;
