...
auto Completion::operator=(Completion &&other)
...
auto Completion::getUserData() const

        noexcept -> unsigned long long {
    return io_uring_cqe_get_data64(this->completion);
}

auto Completion::getResult() const

        noexcept -> int {
    return this->completion->res;
}

auto Completion::getFlags() const

        noexcept -> unsigned int {
    return this->completion->flags;
}
