...
UserRing::UserRing(UserRing &&other)

        noexcept
    : userRing{other.userRing} {
    other.userRing.ring_fd = -1;
}

auto UserRing::operator=(UserRing &&other)
...
auto UserRing::getSelfFileDescriptor() const

        noexcept -> int {
    return this->userRing.ring_fd;
}
...
auto UserRing::forEachCompletion(const

                                 function<auto(io_uring_cqe *cqe)->void> &task) -> unsigned int {
...
auto UserRing::advanceCompletionBufferRingBuffer(io_uring_buf_ring *bufferRing, unsigned int completionCount,
                                                 unsigned short bufferRingBufferCount)
